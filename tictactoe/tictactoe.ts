...
class Player {
  ...
  checkIfPlayerWon(): boolean {
    this.playedPositions.sort(([a], [b]) => a - b);
...
class Game {
  ...
  updatePlayerWonText() {
    ...
    let gameDescText: string = ''
    if (this.playerOne.playerWon) {
      gameDescText = `${this.playerOne.name} won the game`
      if (gameDesc) gameDesc.textContent = gameDescText
    }
    if (this.playerTwo.playerWon) {
      gameDescText = `${this.playerTwo.name} won the game`
      if (gameDesc) gameDesc.textContent = gameDescText
    }
